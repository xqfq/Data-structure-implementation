...
def stack_test():
    ...
    assert s.stackList == [None, None, None],'Initialization failed'
    s.push(1)
    s.push(2)
    s.push(3)
    ...
    assert s.stackList == [1,2,3,4,None,None],'Push method with expansion failed'
    assert s.pop() == 4,'Pop method fails to return the right item'
    assert s.stackList == [1,2,3,None,None,None],'Pop method fails to delete the right item'
